/**
 * Exemplo de functions/index.js utilizando Firebase Functions V2 com configuração de CORS.
 * Documentação:
...
// Em desenvolvimento, usamos process.env.STRIPE_SECRET_KEY; em produção, configure via CLI com 
...
const successUrl = process.env.STRIPE_SUCCESS_URL || 'https://cityswapuk.com/payments/success/{CHECKOUT_SESSION_ID}';
 ...
 * Essa função busca os dados do produto no Firestore (nome e preço), valida o preço e cria uma sessão de pagamento no Stripe.
 * Agora utilizando GBP (£) como moeda.
 */
exports.createCheckoutSession = onCall(
  {
    // Habilita CORS para permitir requisições de outras origens.
    // Se quiser restringir a origens específicas, substitua "true" por um array com os domínios permitidos, ex: ['https://cityswapuk.com']
    cors: true,
  },
  async (data, context) => {
